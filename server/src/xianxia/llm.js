...
突破与时间锁：突破时界面上会出倒计时。你可以每隔一段时间生成一句"时间流逝叙事"——比如灵气运转了几个周天、丹田里的金丹逐渐成形——让等待有画面感。`;
...
const DELTA_LABELS = {
  qi_current: '灵气', health: '生命', spirit_stones: '灵石', fame: '名望',
...
async function processScripted(openai, character, userInput, { script, params }) {
  ...
  const applyOutcome = db.transaction(() => {
    ...
    for (const item of outcome.items || []) {
      db.prepare(
        'INSERT INTO xianxia_items (character_id, name, item_type, grade, description) VALUES (?, ?, ?, ?, ?)'
